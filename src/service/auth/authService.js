...
const login = async ({ email, password }) => {
    try {
        ...
        if (user) {
            ...
            if (user.status === 'Suspended' && !user.tempPassword) {
                throw ApiError.forbidden('User is suspended');
            }
            if (user.isDeleted) {
    ...
    try {
        ...
        if (user) {
            ...
            const updatedUser = await User.findOneAndUpdate({ email },
                { $set: { 'otp.code': otp, 'otp.expiresAt': Date.now() + 4 * 60 * 1000 } },
                { new: true }
            );
            //send mail with 5 digit otp
...
const resetPassword = async (password, id) => {
    ...
    let updated = await user.updateOne({ password: hashPassword, tempPassword: false, status: 'Active' }, { new: true });
