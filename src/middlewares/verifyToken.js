...
function verifyToken(req, res, next) {
  try {
    ...
    jwt.verify(bearerToken, process.env.JWT_SECRET, { ignoreExpiration: true }, function (err, decoded) {
...
async function ioVerifyToken(socket, next) {
  if (socket.handshake.query && socket.handshake.query.token) {
    jwt.verify(socket.handshake.query.token, process.env.JWT_SECRET, { ignoreExpiration: true }, function (err, decoded) {
      if (err) {
